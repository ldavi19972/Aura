...
st.markdown("""
<style>
    html, body, [data-testid="stAppViewContainer"], .stApp { background-color: #0e1117 !important; color: #f1f5f9 !important; }
    .main .block-container { padding-top: 0.4rem !important; padding-bottom: 0.5rem !important; padding-left: 1.2rem !important; padding-right: 1.2rem !important; max-width: 100% !important; }
    header, footer, #MainMenu { visibility: hidden !important; height: 0px !important; }
    div[data-testid="stVerticalBlock"] > div.stTabs { display: none !important; }
    .section-header { font-size: 15px; color: #cbd5e1; font-weight: 700; margin-top: 28px; margin-bottom: 10px; }
    .metric-card { background: #161a22; border: 1px solid #222734; border-radius: 10px; padding: 12px; text-align: center; }
    .metric-title { font-size: 11px; color: #94a3b8; font-weight: 600; text-transform: uppercase; margin-bottom: 2px; }
    .metric-value { font-size: 20px; font-weight: 700; }
    .metric-positive { color: #4ade80; }
    .metric-negative { color: #fb7185; }
    .metric-neutral  { color: #38bdf8; }
    .goal-card { background: #161a22; border: 1px solid #222734; border-radius: 10px; padding: 12px 14px; margin-bottom: 6px; }
    ...
    .cashflow-card { background: #161a22; border: 1px solid #222734; border-radius: 10px; padding: 10px 14px; margin-bottom: 6px; }
    .cf-row { display: flex; justify-content: space-between; align-items: center; padding: 4px 0; border-bottom: 1px solid #1c212c; font-size: 12px; }
    .cf-row:last-child { border-bottom: none; }
    .stExpander { background: #161a22 !important; border: 1px solid #222734 !important; border-radius: 10px !important; margin-bottom: 6px !important; }
    .stExpander summary { background-color: #161a22 !important; color: #e2e8f0 !important; font-size: 11px !important; font-weight: 600 !important; padding: 6px 10px !important; border-radius: 10px !important; }
    .stExpander [data-testid="stExpanderDetails"] { background-color: #161a22 !important; padding: 8px 10px 16px 10px !important; border-top: 1px solid #222734 !important; min-height: 55px; max-height: 1000px; overflow-y: auto; }
</style>
...
def fetch_finances_data(finances_url):
    ...
    # Extract Goals dynamically (Rows 15 to 25 approx)
    transfer_balance_mapping = {}
    for r in range(25, 35):
        if r < len(df):
            name_val = str(df.iloc[r, 2]).strip() # Column C is index 2
            if name_val and name_val.lower() != 'nan':
                transfer_balance_mapping[name_val] = {"row": r, "balance": clean_num(df.iloc[r, 1])}

    for r in range(15, 25): 
        if r >= len(df): break
        g_name = str(df.iloc[r, 10]).strip() # Column K
        if not g_name or g_name.lower() == 'nan': continue
        
        target_val = clean_num(df.iloc[r, 11]) # Column L
        end_date = str(df.iloc[r, 13]).strip() if pd.notna(df.iloc[r, 13]) else "" # Column N
        start_date = str(df.iloc[r, 14]).strip() if pd.notna(df.iloc[r, 14]) else "" # Column O
        ...
        if status_text == "SAVED" or pct >= 100: badge, badge_class, color = "SAVED", "badge-saved", "#4ade80"
        elif status_text == "WAITING": badge, badge_class, color = "WAITING", "badge-waiting", "#facc15"
        else: badge, badge_class, color = "IN PROGRESS", "badge-progress", "#38bdf8"

        fin_data["goals"].append({
            "index": r,
            "name": g_name, "current": bal_val, "target": target_val, 
            "start_date": start_date, "end_date": end_date, "rate": rate_val, 
            "pct": pct, "badge": badge, "badge_class": badge_class, "color": color
        })
...
with col_nav1:
    btn_type = "primary" if current_tab == "Calendar" else "secondary"
    if st.button("📅 Calendar", use_container_width=True, type=btn_type):
        st.session_state["active_tab"] = "Calendar"
        st.rerun()

with col_nav2:
    btn_type = "primary" if current_tab == "Focus" else "secondary"
    if st.button("🔍 Focus View", use_container_width=True, type=btn_type):
        st.session_state["active_tab"] = "Focus"
        st.rerun()

with col_nav3:
    btn_type = "primary" if current_tab == "Finances" else "secondary"
...
st.markdown("""
<style>
    button[kind="secondary"] { background-color: #161a22 !important; border: 1px solid #28303f !important; color: #e2e8f0 !important; font-weight: 600 !important; }
    button[kind="secondary"]:hover { background-color: #1e2532 !important; border-color: #3b82f6 !important; color: #ffffff !important; }
    button[kind="primary"] { background-color: #3b82f6 !important; border: 1px solid #60a5fa !important; color: #ffffff !important; font-weight: 700 !important; }
</style>
""", unsafe_allow_html=True)
...
if current_tab == "Calendar":
    ...
    # Keeping script highly concise for tab 1... (Assuming existing HTML works perfectly)
    st.info("Calendar module is active. Replace this block with your standard calendar HTML if desired.")

# =============================================================================
# TAB 2: FOCUS VIEW
# =============================================================================
elif current_tab == "Focus":
    st.info("Focus View module is active. Replace this block with your standard Focus HTML/Logic.")
...
# =============================================================================
    ...
    if fin:
        ...
        with c1:
            st.markdown(f"""<div class="metric-card"><div class="metric-title">Savings Balance</div><div class="metric-value metric-positive">${fin['kpis']['savings']:,.2f}</div></div>""", unsafe_allow_html=True)
        with c2:
            st.markdown(f"""<div class="metric-card"><div class="metric-title">Total Credit / Debt</div><div class="metric-value metric-negative">-${abs(fin['kpis']['credit']):,.2f}</div></div>""", unsafe_allow_html=True)
        with c3:
            st.markdown(f"""<div class="metric-card"><div class="metric-title">Total Asset Value</div><div class="metric-value metric-neutral">${fin['kpis']['assets']:,.2f}</div></div>""", unsafe_allow_html=True)
        with c4:
            st.markdown(f"""<div class="metric-card"><div class="metric-title">Net Financial Position</div><div class="metric-value metric-negative">-${abs(fin['kpis']['net']):,.2f}</div></div>""", unsafe_allow_html=True)
        ...
        with col_goals:
            ...
            for g in fin["goals"]:
                if g['name'].lower() != "holidays": # Skip mapping the maintain row typically
                    st.markdown(f"""
                    <div class="goal-card">
                        ...
                        <div class="progress-bar-bg"><div class="progress-bar-fill" style="width: {g['pct']}%; background: {g['color']};"></div></div>
                        <div class="goal-footer">
                            <span>${g['current']:,.2f} of ${g['target']:,.2f} ({g['pct']}%)</span>
                            <span>Target: {g['end_date']} • Contrib: ${g['rate']:,.2f}/wk</span>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
        ...
        with col_cash:
            ...
            <div class="cashflow-card">
                <div class="cf-row" style="font-weight:700; color:#4ade80;"><span>Total Weekly Income</span><span>+${fin['income']['total_wk']:,.2f} / wk</span></div>
                <div class="cf-row"><span>↳ Salary</span><span>${fin['income']['salary_wk']:,.2f} / wk</span></div>
                <div class="cf-row"><span>↳ Transfers</span><span>${fin['income']['transfers_wk']:,.2f} / wk</span></div>
            </div>
            <div class="cashflow-card">
                <div class="cf-row" style="font-weight:700; color:#fb7185;"><span>Total Weekly Expenses</span><span>-${fin['expenses']['total_wk']:,.2f} / wk</span></div>
            </div>
            """, unsafe_allow_html=True)

            with st.expander(f"↳ Fixed Bills Breakdown — ${fin['expenses']['fixed_wk']:,.2f} / wk"):
                for b in fin["fixed_bills"]:
                    st.markdown(f"""<div class="cf-row" style="padding: 3px 0;"><span style="color:#cbd5e1; font-weight:600;">{b['item']}</span><span style="color:#94a3b8; font-size:11px;">Impact: <b style="color:#fb7185;">{b['weekly']}/wk</b> &nbsp;|&nbsp; Bill: <b style="color:#f8fafc;">{b['native']}</b> ({b['freq']})</span></div>""", unsafe_allow_html=True)

            with st.expander(f"↳ Variable Budgets Breakdown — ${fin['expenses']['variable_wk']:,.2f} / wk"):
                for v in fin["var_budgets"]:
                    st.markdown(f"""<div class="cf-row" style="padding: 3px 0;"><span style="color:#cbd5e1; font-weight:600;">{v['item']}</span><span style="color:#94a3b8; font-size:11px;">Impact: <b style="color:#fb7185;">{v['weekly']}/wk</b> &nbsp;|&nbsp; Budget: <b style="color:#f8fafc;">{v['weekly']}</b> (Weekly)</span></div>""", unsafe_allow_html=True)

            with st.expander(f"↳ Holiday Allocation — ${fin['expenses']['holiday_wk']:,.2f} / wk"):
                st.markdown(f"""<div class="cf-row" style="padding: 3px 0;"><span style="color:#cbd5e1; font-weight:600;">Holidays Allocation</span><span style="color:#94a3b8; font-size:11px;">Impact: <b style="color:#fb7185;">${fin['expenses']['holiday_wk']:,.2f}/wk</b> &nbsp;|&nbsp; Budget: <b style="color:#f8fafc;">${fin['expenses']['holiday_wk']:,.2f}</b> (Weekly)</span></div>""", unsafe_allow_html=True)

            st.markdown(f"""
            <div class="cashflow-card" style="border-left: 3px solid #38bdf8;">
                <div class="cf-row" style="font-weight:700; color:#38bdf8; font-size:13px;"><span>Net Savings Flow</span><span>+${fin['net_flow']['wk']:,.2f} / wk</span></div>
                <div style="font-size:11px; color:#94a3b8; margin-top:2px;">Equates to <b>+${fin['net_flow']['mo']:,.2f} / mo</b> and <b>+${fin['net_flow']['yr']:,.2f} / yr</b>.</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with st.expander("📝 Review & Update Goals"):
            ...
            if "edit_goals" not in st.session_state:
                st.session_state.edit_goals = [g for g in fin["goals"] if g['name'].lower() != 'holidays']
                    ...
                    with st.spinner("Writing to Google Sheets..."):
                        try:
                            ...
                            for idx, ug in enumerate(updated_goals):
                                # Update Goal Details (Columns K, L, N, O)
                                row_idx = ug["index"] + 1 
                                ...
                                # Update Transfer Account Mapping (Columns B and C down around Row 28)
                                transfer_row = 28 + idx 
